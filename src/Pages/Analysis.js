...
import { ACTIONS, FEED_ACTIONS, SLEEP_ACTIONS, DIAPER_ACTIONS } from '../utils/actions';
...
function BarChart({ data, maxVal, color, label }) {
  ...
  return (
    <div className="bar-chart">
      ...
      {label && <div className="bar-chart-legend" style={{ color }}>{label}</div>}
    </div>
  );
}
...
export default function Analysis({ t, lang, entries, baby }) {
  ...
  const sleepData = keys.map(k => ({
    label: shortLabel(k),
    value: Math.round((byDay[k] || []).filter(e => e.type === 'sleep')
      .reduce((s, e) => s + (e.durationSec || 0), 0) / 360) / 10,
  }));

  // Diaper per day
  const diaperData = keys.map(k => ({
    label: shortLabel(k),
    value: (byDay[k] || []).filter(e => DIAPER_ACTIONS.includes(e.type)).length,
  }));

  // Milk ml per day
  const mlData = keys.map(k => ({
    label: shortLabel(k),
    value: Math.round((byDay[k] || []).reduce((s, e) => s + (e.amountMl || 0), 0)),
  }));

  // Weight trend (last entries)
  const weightEntries = entries.filter(e => e.type === 'weight' && e.weightG).slice(0, 10).reverse();

  // Averages
  const activeDays = keys.filter(k => (byDay[k] || []).length > 0).length || 1;
  const avgFeeds   = (feedData.reduce((s,d) => s+d.value, 0) / activeDays).toFixed(1);
  const avgSleep   = (sleepData.reduce((s,d) => s+d.value, 0) / activeDays).toFixed(1);
  const avgDiapers = (diaperData.reduce((s,d) => s+d.value, 0) / activeDays).toFixed(1);
  const avgMl      = Math.round(mlData.reduce((s,d) => s+d.value, 0) / activeDays);

  // Action breakdown pie-like bar
  const actionBreakdown = Object.entries(ACTIONS).map(([type, a]) => ({
    type, a,
  ...
  return (
    <div className="analysis-page">
      ...
      <div className="analysis-stats">
        <StatCard emoji="🍼" value={avgFeeds} sub={t.avgFeedsPerDay} color="#f9a8d4" />
        <StatCard emoji="😴" value={`${avgSleep}h`} sub={t.avgSleepHours} color="#a78bfa" />
        <StatCard emoji="🩲" value={avgDiapers} sub={t.avgDiapersPerDay} color="#6ee7b7" />
        <StatCard emoji="🥛" value={avgMl > 0 ? `${avgMl}ml` : '—'} sub={lang==='zh'?'平均奶量':'Avg milk'} color="#93c5fd" />
      </div>

      {/* feeding chart */}
      <div className="chart-card">
        <div className="chart-title">{t.feedingTrend}</div>
        <BarChart data={feedData} color="#f9a8d4" label={lang==='zh'?'次數':'feeds'} />
      </div>

      {/* ml chart */}
      {mlData.some(d => d.value > 0) && (
        <div className="chart-card">
          <div className="chart-title">{lang==='zh'?'奶量 (ml)':'Milk (ml)'}</div>
      ...
      <div className="chart-card">
        ...
        <BarChart data={sleepData} color="#a78bfa" label={lang==='zh'?'小時':'hours'} />
      </div>

      {/* diaper chart */}
      <div className="chart-card">
        <div className="chart-title">{t.diaperCount}</div>
        <BarChart data={diaperData} color="#6ee7b7" label={lang==='zh'?'次':'changes'} />
      </div>

      {/* weight trend */}
      {weightEntries.length >= 2 && (
        <div className="chart-card">
          <div className="chart-title">{lang==='zh'?'體重趨勢':'Weight trend'}</div>
      ...
      {actionBreakdown.length > 0 && (
        <div className="chart-card">
          <div className="chart-title">{lang==='zh'?'記錄分佈':'Activity breakdown'}</div>
          {actionBreakdown.map(({ type, a, count }) => (
            <div key={type} className="breakdown-row">
              ...
              <div className="bd-bar-wrap">
                <div className="bd-bar" style={{ width: `${(count/maxCount)*100}%`, background: a.color }} />
              </div>
      ...
      <div className="export-section">
        ...
        <div className="export-btns">
          <button className="export-btn" onPointerUp={() => exportToCSV(entries)}>
            📄 {t.exportCSV}
          </button>
          <button className="export-btn" onPointerUp={() => exportToJSON(entries, baby)}>
            📦 {t.exportJSON}
          </button>
        </div>
      </div>
