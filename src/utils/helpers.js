import { ACTIONS, FEED_ACTIONS, SLEEP_ACTIONS, DIAPER_ACTIONS } from './actions';
...
export function fmtAgeWeeks(w, lang) {
  ...
  if (lang === 'zh') {
    ...
    const months = Math.floor(w/4.33);
    const rem = Math.round(w - months*4.33);
    if (months < 12) return rem > 0 ? `${months}m ${rem}w old` : `${months}mo old`;
    const y = Math.floor(months/12), rm = months%12;
...
export const AGE_GUIDES = [
  ...
  { maxWeeks:Infinity, stageKey:'ageStage7', freqHours:'demand',freqPerDay:'2–3',  bottleMl:'120–180',breastMin:'desired',sleepWake:null},
];
...
export function exportToFormattedText(entries, lang) {
  ...
  for (const [dateKey, dayEntries] of Object.entries(byDay).sort()) {
    ...
    const sleepH = Math.floor(sleepSecs/3600), sleepM = Math.floor((sleepSecs%3600)/60);
...
export function exportToJSON(entries, baby) {
  downloadFile(JSON.stringify({exportedAt:new Date().toISOString(),baby,entries},null,2),
    'baby-diary.json','application/json');
}
