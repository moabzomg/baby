...
import { fmtTime, isSameDay } from "../utils/helpers";
...
export default function Diary({ t, lang, entries, deleteEntry }) {
  ...
  const dayEntries = entries
    .filter((e) => isSameDay(e.timestamp, selectedTs))
      ...
      <div className="diary-left">
        ...
        <div className="cal-nav">
          <button
            className="cal-arrow"
            onPointerUp={() => {
              view === "year" ? setViewYear((y) => y - 1) : prevMonth();
            }}
          >
            ‹
          </button>
          ...
          <button
            className="cal-arrow"
            onPointerUp={() => {
              view === "year" ? setViewYear((y) => y + 1) : nextMonth();
            }}
          >
            ›
          </button>
        ...
        {view === "year" && (
          <div className="year-grid">
            {allMonths.map(({ month, count }) => (
              <button
                key={month}
                className={`year-month-cell ${viewMonth === month ? "selected" : ""}`}
                onPointerUp={() => {
                  setViewMonth(month);
                  setView("month");
                }}
        ...
        {dayEntries.length > 0 && (
          <div className="day-summary-chips">
            {["feed", "sleep", "diaper", "activity", "health"].map((cat) => {
              const catEnt = dayEntries.filter(
                (e) => ACTIONS[e.type]?.category === cat,
              );
              if (!catEnt.length) return null;
              const catAction = Object.values(ACTIONS).find(
                (a) => a.category === cat,
              );
              return (
                <span key={cat} className="summary-chip">
                  ...
                  {cat === "feed" &&
                    catEnt.reduce((s, e) => s + (e.amountMl || 0), 0) > 0 &&
                    ` · ${catEnt.reduce((s, e) => s + (e.amountMl || 0), 0)}ml`}
                  {cat === "sleep" &&
                    catEnt.reduce((s, e) => s + (e.durationSec || 0), 0) > 0 &&
