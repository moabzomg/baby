...
import { fmtTime, fmtDateKey, isSameDay, getSleepIssues } from '../utils/helpers';
...
function buildCalendar(y,m){
  const days=getDaysInMonth(y,m), first=getFirstDOW(y,m);
  const cells=[];
...
export default function Diary({ t, lang, entries, deleteEntry }) {
  ...
  const dayEntries = entries.filter(e=>isSameDay(e.timestamp,selTs)).sort((a,b)=>a.timestamp-b.timestamp);
  ...
  for(const e of [...dayEntries].sort((a,b)=>a.timestamp-b.timestamp)){
    ...
    if(e.type==='wake'&&lastSleepTs){ sleepDurations[e.id]=Math.floor((e.timestamp-lastSleepTs)/1000); lastSleepTs=null; }
  }

  // Day chips summary
  const todayFeeds   = dayEntries.filter(e=>FEED_ACTIONS.includes(e.type));
  const todayMl      = todayFeeds.reduce((s,e)=>s+(e.amountMl||0),0);
  const todayDiapers = dayEntries.filter(e=>DIAPER_ACTIONS.includes(e.type)).length;
  const sleepSecs    = Object.values(sleepDurations).reduce((a,b)=>a+b,0);

  const allMonths = Array.from({length:12},(_,m)=>({
    m, count: entries.filter(e=>{ const d=new Date(e.timestamp); return d.getFullYear()===vy&&d.getMonth()===m; }).length,
  }));
    ...
    <div className="diary-layout">
      ...
      <div className="diary-left">
        ...
        <div className="diary-view-switch">
          {['day','month','year'].map(v=>(
            <button key={v} className={`view-switch-btn ${view===v?'active':''}`} onPointerUp={()=>setView(v)}>
              {v==='day'?zh?'日':'Day':v==='month'?zh?'月':'Mon':zh?'年':'Year'}
            </button>
          ))}
        </div>
        ...
        <div className="cal-nav">
          <button className="cal-arrow" onPointerUp={()=>view==='year'?setVy(y=>y-1):prevMonth()}>‹</button>
          <span className="cal-title">{view==='year'?vy:`${monthNames[vm]} ${vy}`}</span>
          <button className="cal-arrow" onPointerUp={()=>view==='year'?setVy(y=>y+1):nextMonth()}>›</button>
        </div>
        ...
        {view==='year' && (
          <div className="year-grid">
            {allMonths.map(({m,count})=>(
              <button key={m} className={`year-month-cell ${vm===m?'selected':''}`}
                onPointerUp={()=>{setVm(m);setView('month');}}>
        ...
        {(view==='month'||view==='day') && (
          <>
            <div className="cal-weekdays">{dayNames.map(d=><span key={d} className="cal-weekday">{d}</span>)}</div>
            <div className="cal-grid">
              {cells.map((day,i)=>{
                if(!day) return <div key={`e${i}`}/>;
                const dEnt = entries.filter(e=>isSameDay(e.timestamp,new Date(vy,vm,day).getTime()));
                ...
                return (
                  <button key={day} className={`cal-day ${isToday?'today':''} ${isSel?'selected':''}`}
                    onPointerUp={()=>{setVd(day);setView('day');}}>
                    <span className="cal-day-num">{day}</span>
                    {icons.length>0&&<span className="cal-day-icons">{icons.join('')}</span>}
      ...
      <div className="diary-right">
        <div className="diary-day-header">
          <span className="diary-day-title">{vy}/{String(vm+1).padStart(2,'0')}/{String(vd).padStart(2,'0')}</span>
          <span className="diary-day-count">{dayEntries.length} {zh?'條記錄':'entries'}</span>
        </div>

        {/* summary chips */}
        {dayEntries.length>0&&(
          <div className="day-summary-chips">
            {todayFeeds.length>0&&<span className="summary-chip">🍼 {todayFeeds.length}{todayMl>0?` · ${todayMl}ml`:''}</span>}
            {sleepSecs>0&&<span className="summary-chip">😴 {Math.round(sleepSecs/360)/10}h</span>}
            {todayDiapers>0&&<span className="summary-chip">🩲 {todayDiapers}</span>}
            {sleepIssues.size>0&&dayEntries.some(e=>sleepIssues.has(e.id))&&
              <span className="summary-chip summary-chip--warn">⚠ {zh?'睡眠異常':'Sleep issue'}</span>}
          </div>
        )}
            ...
            <ul className="timeline">
              {dayEntries.map(e=>{
                ...
                if(e.type==='breastfeed'){
                  ...
                  detail=parts.join(' / ');
                } else if(e.type==='wake'&&dur) detail=`(${Math.floor(dur/3600)}h${Math.floor((dur%3600)/60)}m)`;
                ...
                return (
                  <li key={e.id} className="timeline-item">
                    ...
                    <div className={`timeline-content ${warn?'timeline-content--warn':''}`}>
                      <div className="tl-header">
                        <span className="tl-emoji">{a.emoji}</span>
                        <span className="tl-name">{zh?a.labelZh:a.labelEn}</span>
                      ...
                      {warn&&<div className="tl-warn-msg">{zh?'⚠ 睡眠記錄異常，請檢查並修正':'⚠ Sleep record issue — please review'}</div>}
                    </div>
                  </li>
                );
              })}
