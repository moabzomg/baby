...
const WHO_WEIGHT = {
  ...
  36: [12000, 13500, 14700, 16500, 17500],
  48: [13700, 15300, 16700, 18700, 20000],
  60: [15500, 17200, 19200, 21500, 23000],
  72: [17000, 19000, 21400, 24000, 25800],
  84: [18800, 21000, 23800, 27000, 29000],
  96: [20500, 23000, 26400, 30200, 32700],
  108: [22200, 25200, 29300, 34000, 37000],
  120: [24000, 27500, 32500, 38500, 42500],
  132: [26000, 30200, 36000, 43500, 48500],
  144: [28500, 33500, 40000, 49000, 55000],
  156: [31500, 37000, 45000, 55500, 63000],
  168: [36000, 42000, 52000, 64000, 73000],
  180: [41000, 48000, 60000, 74000, 84000],
  192: [46500, 54500, 67500, 83000, 94000],
  204: [52000, 61000, 75000, 91000, 103000],
...
const WHO_HEIGHT = {
  ...
  36: [88.7, 91.9, 96.1, 100.0, 103.0],
  48: [94.9, 98.7, 103.3, 107.8, 111.1],
  60: [100.7, 104.9, 110.0, 115.0, 118.7],
  72: [106.1, 110.8, 116.4, 122.0, 126.2],
  84: [111.2, 116.2, 122.4, 128.6, 133.2],
  96: [116.2, 121.5, 128.3, 135.2, 140.2],
  108: [120.9, 126.6, 134.0, 141.5, 147.0],
  120: [125.6, 131.6, 139.6, 147.8, 153.8],
  132: [130.3, 136.8, 145.2, 154.1, 160.6],
  144: [135.1, 141.9, 150.9, 160.4, 167.4],
  156: [140.1, 147.3, 156.6, 166.8, 174.3],
  168: [145.2, 152.7, 162.2, 172.8, 180.7],
  180: [150.2, 158.0, 167.8, 178.7, 186.9],
};
const WHO_HEAD = {
              ...
              {s.data.length > 1 && (
                <polyline
                  points={s.data
                    .map(
                      (d, i) =>
                        `${toX(i)},${padT + iH - (d.value / Math.max(...s.data.map((x) => x.value), 1)) * iH}`,
                    )
                    .join(" ")}
                  fill="none"
                  stroke={s.color}
...
function TempLineChart({ entries, lang }) {
  ...
  const temps = useMemo(
    () =>
      entries
        .filter((e) => e.type === "temp" && e.valueNum != null)
  ...
  if (temps.length === 0)
    return (
      <div className="chart-empty">
        {zh ? "尚無體溫記錄" : "No temperature records"}
      </div>
    );
  ...
  const minV = Math.min(...vals, 36) - 0.2,
    maxV = Math.max(...vals, 38) + 0.2;
  const n = temps.length;
  const toX = (i) => padL + (i / Math.max(n - 1, 1)) * iW;
  const toY = (v) => padT + iH - ((v - minV) / (maxV - minV)) * iH;
  const NORMAL_Y_TOP = toY(37.5),
    NORMAL_Y_BOT = toY(36.0);
    ...
    <div className="temp-line-chart">
      <svg viewBox={`0 0 ${W} ${H}`} style={{ width: "100%", height: "auto" }}>
        ...
        <rect
          ...
          y={NORMAL_Y_TOP}
          width={iW}
          height={NORMAL_Y_BOT - NORMAL_Y_TOP}
          fill="#22c55e"
          opacity={0.08}
        />
        {/* Lines */}
        ...
        {[0, temps.length - 1].map((i) => (
          <text
            ...
            fill="var(--text-tertiary)"
          >
            {new Date(temps[i].timestamp).toLocaleDateString([], {
              month: "numeric",
              day: "numeric",
            })}
          </text>
      ...
      <div className="dot-legend" style={{ marginTop: 4 }}>
        {[
          { c: "#60a5fa", l: zh ? "低温" : "Low" },
          { c: "#22c55e", l: zh ? "正常" : "Normal" },
          { c: "#f59e0b", l: zh ? "低燒" : "Low fever" },
          { c: "#ef4444", l: zh ? "發燒" : "Fever" },
        ].map((x) => (
          <span key={x.l} className="dot-legend-item">
            <span className="dot-legend-dot" style={{ background: x.c }} />
            {x.l}
          </span>
        ))}
      </div>
...
function DiaperChart({ entries, dayKeys, lang, zh }) {
  ...
  const types = [
    { id: "pee", color: "#60a5fa", label: zh ? "小便" : "Pee" },
    { id: "poo", color: "#d97706", label: zh ? "大便" : "Poo" },
    { id: "pee_poo", color: "#86efac", label: zh ? "大小便" : "Both" },
  ];
  ...
  const countData = types.map((ty) => ({
    ...
    data: dayKeys.map((k) => ({
      ...
      value: entries.filter(
        (e) => fmtDateKey(e.timestamp) === k && e.type === ty.id,
      ).length,
    })),
  }));
    ...
    return (
      <>
        <div className="diaper-view-toggle">
          <button
            className={`diaper-toggle-btn ${view === "count" ? "active" : ""}`}
            onPointerUp={() => setView("count")}
          >
            {zh ? "次數" : "Count"}
          </button>
          <button
            className={`diaper-toggle-btn ${view === "time" ? "active" : ""}`}
            onPointerUp={() => setView("time")}
          >
            {zh ? "時間" : "Time"}
          </button>
        </div>
        {types.map((ty) => (
          <div key={ty.id} style={{ marginBottom: 12 }}>
            ...
            <BarChart
              data={countData.find((d) => d.id === ty.id)?.data || []}
              color={ty.color}
              unit=""
              h={60}
            />
  ...
  entries
    .filter(
      (e) =>
        ...
        dayKeys.includes(fmtDateKey(e.timestamp)),
    )
  ...
  return (
    <>
      ...
      <div
        style={{ display: "flex", gap: 4, marginBottom: 4, paddingLeft: 36 }}
      >
        {["0h", "6h", "12h", "18h", "24h"].map((l) => (
          <span
            key={l}
...
function FoodChart({ entries, dayKeys, lang, zh }) {
    ...
    return (
      <>
        <div className="food-view-toggle">
          <button
            className={`food-toggle-btn ${view === "time" ? "active" : ""}`}
            onPointerUp={() => setView("time")}
          >
            {zh ? "時間" : "Time"}
          </button>
          <button
            className={`food-toggle-btn ${view === "amount" ? "active" : ""}`}
            onPointerUp={() => setView("amount")}
          >
            {zh ? "奶量" : "Amount"}
          </button>
        </div>
          ...
          >
            ...
            <div style={{ flex: 1, height: 20, position: "relative" }}>
              {(feedsByDay[k] || []).map((e, i) => {
                const dayStart = new Date(k + "T00:00:00").getTime();
                const pct = ((e.timestamp - dayStart) / 86400000) * 100;
                const ty = FEED_TYPES_VIZ.find((t) => t.id === e.type) || {
                  color: "#888",
                };
                const filled =
                  e.amountMl != null || e.breastL != null || e.breastR != null;
                return (
                  <div
                    key={i}
                    style={{
                      ...
                      height: 9,
                      borderRadius: "50%",
                      background: filled ? ty.color : "transparent",
                      border: `2px solid ${ty.color}`,
                    }}
                  />
                );
              })}
  ...
  const mlData = dayKeys.map((k) => ({
    label: shortLbl(k),
    value: entries
      .filter(
        (e) =>
          fmtDateKey(e.timestamp) === k &&
          ["formula", "ebm", "bottle", "drink"].includes(e.type),
      )
      .reduce((s, e) => s + (e.amountMl || 0), 0),
  }));
  const bfLData = dayKeys.map((k) => ({
    label: shortLbl(k),
    value: entries
      .filter((e) => fmtDateKey(e.timestamp) === k && e.type === "breastfeed")
      .reduce((s, e) => s + (e.breastL || 0), 0),
  }));
  const bfRData = dayKeys.map((k) => ({
    label: shortLbl(k),
    value: entries
      .filter((e) => fmtDateKey(e.timestamp) === k && e.type === "breastfeed")
      .reduce((s, e) => s + (e.breastR || 0), 0),
  }));
  const solidCnt = dayKeys.map((k) => ({
    label: shortLbl(k),
    value: entries.filter(
      (e) =>
        fmtDateKey(e.timestamp) === k &&
        ["solid", "snack", "meal"].includes(e.type),
    ).length,
  }));
    ...
    <>
      ...
      <div style={{ display: "flex", gap: 10 }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          >
            {zh ? "母乳 L+R (分鐘)" : "BF L+R (min)"}
          </div>
          <BarChart
            data={bfLData.map((d, i) => ({
              label: d.label,
              value: d.value + bfRData[i].value,
            }))}
            color="#f9a8d4"
            unit="m"
            h={80}
...
function GrowthChart({ entries, baby, metric, lang }) {
  ...
  const birthday = useMemo(
    () => (baby?.date ? new Date(baby.date) : null),
    [baby?.date],
  );
  ...
  const measurements = useMemo(
    () =>
      entries
        .filter((e) => e.type === metric && e.valueNum != null)
        .sort((a, b) => a.timestamp - b.timestamp)
        .map((e) => ({
          ts: e.timestamp,
          ageMonths: birthday
    ...
    return (
      <div className="chart-empty">
        {zh
          ...
          : `No ${ACTIONS[metric]?.labelEn} records`}
      </div>
    );

  const hasWho = !!whoMap[metric] && !!birthday;
  const maxAgeM = hasWho
  ...
  const toY = (val) =>
    pT +
    iH -
    ((val - (minVal - padV)) / (maxVal + padV - (minVal - padV))) * iH;

  const whoAges = [
    0, 1, 2, 3, 4, 6, 9, 12, 18, 24, 36, 48, 60, 72, 84, 96, 108, 120, 132, 144,
    156, 168, 180,
  ].filter((a) => a <= (maxAgeM || 0) + 6 && whoMap[metric]?.[a]);
  const whoP = hasWho
    ? [0, 2, 4].map((idx) =>
        whoAges.map((a) => {
          const ref = getWhoRef(whoMap[metric], a);
          return { x: toX(a), y: toY(ref[idx]) };
        }),
      )
    : [];
      ...
      <svg viewBox={`0 0 ${W} ${H}`} style={{ width: "100%", height: "auto" }}>
        {hasWho && whoP.length === 3 && (
          <>
            <polygon
              points={[
                ...whoP[0].map((p) => `${p.x},${p.y}`),
                ...whoP[2].map((p) => `${p.x},${p.y}`).reverse(),
              ].join(" ")}
...
export default function Analysis({ t, lang, entries, baby }) {
  ...
  const avgFeeds = (
    buildSeries("feeds").reduce((s, d) => s + d.value, 0) / activeDays
  ).toFixed(1);
  const avgMl = Math.round(
    buildSeries("milk").reduce((s, d) => s + d.value, 0) / activeDays,
  );
  const avgSleep = (
    buildSeries("sleep").reduce((s, d) => s + d.value, 0) / activeDays
  ).toFixed(1);
  const avgDiapers = (
    buildSeries("diapers").reduce((s, d) => s + d.value, 0) / activeDays
  ).toFixed(1);
    ...
    <div className="analysis-page">
          ...
          <div className="chart-card">
            <div className="chart-card-header">
              <div className="chart-title">
                {zh ? "綜合圖表" : "Combined chart"}
              </div>
              <div style={{ display: "flex", gap: 6 }}>
                <button
                  className="range-btn active"
                  onPointerUp={() =>
                    setActive(new Set(CHART_CONFIGS.map((c) => c.id)))
                  }
            ...
            {activeCharts.size > 0 && (
              <CombinedChart
                series={CHART_CONFIGS.filter((c) => activeCharts.has(c.id)).map(
                  (c) => ({ ...c, data: buildSeries(c.id) }),
                )}
                keys={keys}
                zh={zh}
              />
      ...
      {activeView === "sleep" && (
        <>
          <div className="chart-card">
            <div className="chart-title">
              {zh ? "睡眠時長" : "Sleep duration"}
            </div>
            <BarChart
              data={buildSeries("sleep")}
              color="#a78bfa"
              unit="h"
              h={100}
            />
          ...
          <div className="chart-card">
            <div className="chart-title">
              {zh ? "睡眠時段（每天）" : "Sleep timeline (per day)"}
            </div>
