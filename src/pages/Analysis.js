...
import { fmtDateKey, exportToCSV, exportToJSON, exportToFormattedText,
         importFromJSON, isSameDay } from '../utils/helpers';

// ── WHO growth reference data (simplified percentiles) ────────────────────
const WHO_WEIGHT = { // age months → [P3, P15, P50, P85, P97] in grams
  0:[2500,2800,3300,3900,4300], 1:[3400,3800,4500,5100,5700],
  2:[4300,4900,5600,6300,6800], 3:[5000,5700,6400,7100,7700],
  4:[5600,6200,7000,7800,8400], 6:[6400,7100,7900,8800,9500],
  9:[7200,8000,8900,9900,10600],12:[7800,8700,9600,10800,11500],
  18:[8800,9800,10900,12200,13000],24:[9700,10900,12200,13500,14500],
};
const WHO_HEIGHT = { // age months → [P3, P15, P50, P85, P97] in cm
  0:[46.3,47.9,49.9,52.0,53.4], 1:[50.8,52.5,54.7,56.9,58.5],
  2:[54.4,56.3,58.4,60.7,62.4], 3:[57.3,59.4,61.4,63.8,65.5],
  4:[59.7,61.8,63.9,66.3,68.0], 6:[63.3,65.6,67.6,70.3,72.1],
  9:[68.0,70.1,72.3,75.0,77.1],12:[71.7,73.9,75.7,78.8,81.2],
  18:[77.5,80.0,82.3,85.4,87.7],24:[82.5,85.1,87.8,91.1,93.6],
};
const WHO_HEAD = { // age months → [P3, P15, P50, P85, P97] in cm
  0:[31.7,32.8,34.5,36.2,37.3], 1:[34.4,35.5,37.3,39.0,40.1],
  2:[36.2,37.4,39.1,40.8,41.9], 3:[37.7,38.9,40.5,42.2,43.3],
  6:[41.0,42.2,43.8,45.5,46.5], 9:[43.0,44.2,45.8,47.5,48.5],
  12:[44.5,45.8,47.2,48.9,49.9],18:[46.1,47.4,48.9,50.5,51.5],
...
function GrowthChart({ entries, baby, metric, lang }) {
  const zh = lang === 'zh';
  const birthday = baby?.date ? new Date(baby.date) : null;

  const actionMap = { weight:'weight', height:'height', head:'head', chest:'chest' };
  const unitMap   = { weight:'g', height:'cm', head:'cm', chest:'cm' };
  const whoMap    = { weight:WHO_WEIGHT, height:WHO_HEIGHT, head:WHO_HEAD };

  const measurements = useMemo(() =>
    entries
      .filter(e => e.type === actionMap[metric] && e.valueNum != null)
      .sort((a,b) => a.timestamp - b.timestamp)
      .map(e => ({
        ts: e.timestamp,
        ageMonths: birthday ? (e.timestamp - birthday.getTime()) / (30.44*24*3600*1000) : null,
        value: e.valueNum,
      })),
  ...
  if (measurements.length === 0) {
    return <div className="chart-empty">{zh?`尚無${ACTIONS[metric]?.labelZh}記錄`:`No ${ACTIONS[metric]?.labelEn} records yet`}</div>;
  }

  const hasWho = whoMap[metric] && birthday;
  const maxAge = hasWho ? Math.max(...measurements.map(m=>m.ageMonths), 24) : null;
  const maxVal = Math.max(...measurements.map(m=>m.value)) * 1.1;

  const H = 160, W = 100; // viewBox units (%)
  const padL = 12, padR = 4, padT = 8, padB = 20;
  ...
  // WHO reference bands
  const whoPercentiles = hasWho ? [3,50,97].map((_, idx) => {
    const ages = [0,1,2,3,4,6,9,12,18,24].filter(a=>a<=maxAge+2);
    return ages.map(a => {
    ...
    <div className="growth-chart">
      <svg viewBox={`0 0 ${W} ${H}`} style={{width:'100%',height:'auto'}}>
        ...
        {hasWho && whoPercentiles.length === 3 && (
          <>
            <polygon
              points={[...whoPercentiles[0].map(p=>`${p.x},${p.y}`), ...whoPercentiles[2].map(p=>`${p.x},${p.y}`).reverse()].join(' ')}
              fill={ACTIONS[metric]?.color || '#93c5fd'} opacity={0.12}
            />
            {[0,1,2].map((idx,i) => (
              <polyline key={i} points={whoPercentiles[idx].map(p=>`${p.x},${p.y}`).join(' ')}
                stroke={ACTIONS[metric]?.color||'#93c5fd'} strokeWidth={0.4} fill="none"
                strokeDasharray={idx===1?'':'2,1'} opacity={0.6} />
            ))}
          </>
        )}
        {/* Measurement line */}
        ...
        {pts.map((p,i) => (
          <g key={i}>
            ...
            <text x={p.x} y={p.y-2.5} textAnchor="middle" fontSize={2.5} fill="#6b7280">
              {metric==='weight' ? (p.value/1000).toFixed(1)+'kg' : p.value+unitMap[metric]}
            </text>
          </g>
        ))}
        {/* Axes */}
        <line x1={padL} y1={padT+innerH} x2={padL+innerW} y2={padT+innerH} stroke="#e5e7eb" strokeWidth={0.4}/>
        <line x1={padL} y1={padT} x2={padL} y2={padT+innerH} stroke="#e5e7eb" strokeWidth={0.4}/>
      </svg>
      {hasWho && (
...
// ── Chart config ──────────────────────────────────────────────────────────
const CHART_CONFIGS = [
  { id:'feeds',   labelZh:'餵食次數', labelEn:'Feeds',        color:'#f9a8d4', unit:'' },
  { id:'milk',    labelZh:'奶量ml',   labelEn:'Milk (ml)',    color:'#93c5fd', unit:'ml'},
  { id:'sleep',   labelZh:'睡眠',     labelEn:'Sleep (h)',    color:'#a78bfa', unit:'h' },
  { id:'diapers', labelZh:'尿片',     labelEn:'Diapers',      color:'#6ee7b7', unit:'' },
  { id:'bf_l',    labelZh:'母乳左',   labelEn:'BF Left (m)',  color:'#fda4af', unit:'m' },
  { id:'bf_r',    labelZh:'母乳右',   labelEn:'BF Right (m)', color:'#f9a8d4', unit:'m' },
];
...
export default function Analysis({ t, lang, entries, baby }) {
  ...
  const byDay = useMemo(() => {
    const map = {};
    entries.forEach(e=>{ const k=fmtDateKey(e.timestamp); if(!map[k])map[k]=[]; map[k].push(e); });
    return map;
  },[entries]);

  const shortLbl = k => { const [,m,d]=k.split('-'); return `${parseInt(m)}/${parseInt(d)}`; };

  const buildData = (id) => keys.map(k => {
    const day = byDay[k]||[];
    let val=0;
  ...
  const avgFeeds   = (buildData('feeds').reduce((s,d)=>s+d.value,0)/activeDays).toFixed(1);
  const avgMl      = Math.round(buildData('milk').reduce((s,d)=>s+d.value,0)/activeDays);
  const avgSleep   = (buildData('sleep').reduce((s,d)=>s+d.value,0)/activeDays).toFixed(1);
  const avgDiapers = (buildData('diapers').reduce((s,d)=>s+d.value,0)/activeDays).toFixed(1);
  ...
  const handleImport = (e) => {
    ...
    reader.onload = ev => {
      try {
        ...
        setImportError(zh ? '匯入失敗：格式錯誤' : 'Import failed: invalid format');
      }
    };
  ...
  return (
    <div className="analysis-page">
      ...
      <div className="range-toggle">
        <button className={`range-btn ${range==='7'?'active':''}`} onPointerUp={()=>setRange('7')}>
          {zh?'過去7天':'Last 7 days'}
        </button>
        <button className={`range-btn ${range==='30'?'active':''}`} onPointerUp={()=>setRange('30')}>
      ...
      <div className="chart-card">
        <div className="chart-card-header">
          ...
          <div style={{display:'flex',gap:6}}>
            <button className="range-btn active" onPointerUp={showAll} style={{padding:'4px 10px',fontSize:12}}>{zh?'全選':'All'}</button>
            <button className="range-btn" onPointerUp={hideAll} style={{padding:'4px 10px',fontSize:12}}>{zh?'清除':'None'}</button>
          </div>
        </div>
        <div className="chart-toggles">
          {CHART_CONFIGS.map(c=>(
            <button key={c.id}
              ...
              style={activeCharts.has(c.id)?{background:c.color,borderColor:c.color,color:'#111'}:{}}
        ...
        <div className="growth-metric-tabs">
          {GROWTH_ACTIONS.map(id=>(
            <button key={id}
              ...
              style={growthMetric===id?{background:ACTIONS[id]?.color,borderColor:ACTIONS[id]?.color}:{}}
